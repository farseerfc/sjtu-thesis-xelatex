...
JPG或者PNG格式的图片，必须先用命令{\textbf{ebb file.jpg}}确定边框——生成.bb (Bounding Box) 文件供~latex~使用，然后才可以插入。
需要注意的是，ebb生成的.bb文件中，边界不准确，需要自己用记事本打开.bb文件填入正确的文件边界，请参考模板附带的.bb文件。
插入PNG/JPG的例子如\ref{fig:SRR}所示。
这两个水平并列放置的图共享一个``图标题''(table caption)，没有各自的小标题。

\begin{figure}[!htp]
  \centering
  \includegraphics[scale=0.4]{chap2/fig1}
    \includegraphics[scale=0.4]{chap2/fig2}
    \bicaption[fig:SRR]{这里将出现在插图索引中}{中文题图}{Fig}{English caption}
\end{figure}

这里还有插入eps图像和pdf图像的例子，如图\ref{fig:pdfeps}。
pdf图像同样需要命令{\textbf{ebb file.pdf}}确定边框，然后再插入。
由于ebb能力有限，只能处理v1.3版本的PDF，所以生成PDF的时候应该注意兼容性问题。
下面插入一个EPS和PDF格式的图片，并且作为并列子图插入，每个子图有自己的小标题。
并列子图的功能是使用subfigure宏包实现的。
...
效果以最后生成的pdf文件为准，dvi预览时，通常只能显示“边框”而不能显示图像内容。
...
然后学术搜索找到文献下会有``导出到BibTeX''连接，点击后Firefox会打开新的标签页，出现类似下面的内容
...
请注意，这个条目离``规范''还有一些距离。
  ...
  \begin{lstlisting}[caption={从Google Scholar找到的，但并不规范的.bib条目}, escapeinside="", numbers=none]
  ...
  上面的.bib条目的``名字''——``白2008信用风险传染模型和信用衍生品的定价''，包含~ASCII~以外的字符，BibTeX无法处理；
  ...
  显然，这里描述的是参考文献的内容，而不是表现形式。
    ...
    \begin{lstlisting}[caption={一个符合规范的.bib条目}, escapeinside="", numbers=none]
    @phdthesis{bai2008,
      title={{"信用风险传染模型和信用衍生品的定价"}},
      author={"白云芬"},
      year={2008},
...
硕士学位论文\cite{zhubajie,metamori2004}，博士学位论文\upcite{shaheshang,FistSystem01,bai2008}，标准文件\cite{IEEE-1363}，技术报告\upcite{NPB2}。

最后总结一些注意事项：
\begin{itemize}
\item 参考文献只有在正文中被引用了，才会在最后的参考文献列表中出现；
\item 参考文献``数据库文件''.bib是纯文本文集，请使用GBK编码，不要使用UTF-8编码，这个模板无法处理UTF-8编码的中文字符；
...
参考文献数据库.bib虽然是纯文本的，可以用任意的文本编辑器查看，但总有人喜欢一个找一个``条目化''地查看每一条参考文献。
